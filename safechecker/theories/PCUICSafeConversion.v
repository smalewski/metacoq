...
(*  Local Definition gΣ := abstract_env_rel Σ. *)
  ...
  Proof.
    ...
    - intros x y [e]; eauto.  constructor. eapply eq_term_sym.
      erewrite abstract_env_irr; eauto.
    - intros x y z [e1] [e2]; eauto. constructor. 
      erewrite abstract_env_irr; try apply wfΣ; eauto.  eapply eq_term_trans. all: eauto.
    - intro u. eapply Subterm.wf_lexprod.
      + intro. eapply posR_Acc.
      + intros [w' q'].
        unshelve eapply dlexmod_Acc.
        * intros x y [e]; eauto. constructor.
        erewrite abstract_env_irr; try apply wfΣ; eauto. eapply eq_term_sym. assumption.
        * intros x y z [e1] [e2]; eauto. constructor. 
        erewrite abstract_env_irr; try apply wfΣ; eauto. eapply eq_term_trans. all: eauto.
        ...
        * intros x. exists (fun  _ _ => sq (eq_term_refl _ _ _)).
          intros [[q'' h] ?].
    ...
    - intros x x' y [e] [y' [x'' [r [[e1] [e2]]]]]; eauto.
      ...
      constructor. eapply eq_term_trans. all: eauto.
    - intros x. exists (fun  _ _ => sq (eq_term_refl _ _ _)). intros [[q' h] [? [? ?]]].
  ...
  Definition Ret s Γ t π t' π' :=
    forall (leq : conv_pb),
      ...
      | Args => ConversionResult (forall Σ, abstract_env_rel X Σ -> ∥equality_terms Σ (Γ,,, stack_context π)
                                   (decompose_stack π).1
                                   (decompose_stack π').1∥)
      end.
  ...
  *)
  ...
  Lemma conv_pb_relb_complete Σ (wfΣ : abstract_env_rel X Σ) leq u u' :
    wf_universe Σ u ->
    wf_universe Σ u' ->
...
Qed.
  ...
  Lemma conv_pb_relb_make_complete Σ (wfΣ : abstract_env_rel X Σ) leq x y :
  ...
  Proof.
    intros wfx wfy r.
    eapply conv_pb_relb_complete; eauto.
  ...
  Proof.
    ...
    - destruct u'; [easy|].
      ...
      split.
      + eapply (conv_pb_relb_make_complete _ _ Conv); eauto.
      + apply IHu; eauto. 
  Unshelve. all:eauto.   
  Qed.
  ...
  Proof.
    ...
    - eapply conv_pb_relb_make_complete; eauto.
    - eapply (conv_pb_relb_make_complete _ _ Conv); eauto.
    Unshelve. eauto. 
  Qed.
  ...
  Proof.
    intros r. pose proof (hΣ _ wfΣ). sq. now apply red_equality.
  Qed.

  Lemma red_conv_cum_r {leq Γ u v Σ}{wfΣ : abstract_env_rel X Σ} :
    Σ ;;; Γ ⊢ u ⇝ v -> conv_cum leq Σ Γ v u.
  Proof.
    intros r. pose proof (hΣ _ wfΣ). sq. now apply red_equality_inv.
  Qed.
...
(*  
  ...
  Equations (noeqns) isconv_context_aux
            ...
            (check :
               forall (leq : conv_pb) (Δh : context_hole) (t : term) (Δh' : context_hole) (t' : term),
                 ...
                 Δ' = fill_context_hole Δh' t' ->
                 (forall Σ (wfΣ : abstract_env_rel X Σ), ∥ context_equality_rel false Σ Γ (context_hole_context Δh) (context_hole_context Δh')∥) ->
            ...
            (eq : Δ = Δpre ,,, Δpost)
            (eq' : Δ' = Δ'pre ,,, Δ'post) :
    ConversionResult (forall Σ (wfΣ : abstract_env_rel X Σ), ∥context_equality_rel false Σ Γ Δpre Δ'pre∥) by struct Δpre := {
  ...
  Definition isconv_context
            ...
            (check :
               forall (leq : conv_pb) (Δh : context_hole) (t : term) (Δh' : context_hole) (t' : term),
                 ...
                 (forall Σ (wfΣ : abstract_env_rel X Σ),  ∥context_equality_rel false Σ Γ (context_hole_context Δh) (context_hole_context Δh')∥) ->
                 ConversionResult (forall Σ (wfΣ : abstract_env_rel X Σ), conv_cum leq Σ (Γ,,, context_hole_context Δh) t t'))
    : ConversionResult (forall Σ (wfΣ : abstract_env_rel X Σ), ∥context_equality_rel false Σ Γ Δ Δ'∥) :=
  ...
  (hp : forall Σ (wfΣ : abstract_env_rel X Σ), ∥ equality_predicate Σ (Γ ,,, stack_context π) p p' ∥)
  (h1 : forall Σ (wfΣ : abstract_env_rel X Σ), ∥ equality_brs Σ (Γ ,,, stack_context π) p brs1 brs1' ∥) :
  ∥ ∑ mdecl idecl,
  ...
  Equations isconv_branches (Γ : context)
    ...
    : ConversionResult (forall Σ (wfΣ : abstract_env_rel X Σ), ∥ equality_brs Σ (Γ ,,, stack_context π) p brs2 brs2' ∥)
    by struct brs2 :=
  ...
  Equations isconv_branches' (Γ : context)
    ...
    : ConversionResult (forall Σ (wfΣ : abstract_env_rel X Σ), ∥ equality_brs Σ (Γ ,,, stack_context π) p brs brs' ∥) :=
  ...
  Next Obligation.
    ...
    { apply All2_map. eapply All2_impl. 1: eassumption.
      ...
      specialize (e _ wfΣ). simpl in *. split; tas.
    }
    clear ha.
    revert h.
  ...
  Proof.
    ...
    2:{ symmetry in X0. eapply context_equality_forget in X0. exact X0. }
  ...
  Proof.
    ...
    * destruct H; fvs. 
    * fvs.
    * destruct H. rewrite <-(All2_fold_length X0). now eapply equality_is_open_term_right.
  Qed.
  ...
  Proof.
    ...
    2:{ eapply whnf_conv_context; eauto.
        symmetry in X0. now eapply context_equality_forget in X0. }
  ...
  Equations (noeqns) isconv_predicate_params_aux
    ...
    ConversionResult (forall Σ (wfΣ : abstract_env_rel X Σ), ∥equality_terms Σ (Γ,,, stack_context π1) post1 post2∥) :=
  ...
  Equations (noeqns) isconv_predicate
            ...
            (eqci : ci1 = ci2)
            (aux : Aux Term Γ (tCase ci1 p1 c1 brs1) π1 (tCase ci2 p2 c2 brs2) π2 h2)
    : ConversionResult (forall Σ (wfΣ : abstract_env_rel X Σ), ∥equality_predicate Σ (Γ,,, stack_context π1) p1 p2∥) :=
...
Qed.
  ...
  Equations(noeqns) _isconv_args' (leq : conv_pb) (Γ : context)
    ...
    : ConversionResult (forall Σ (wfΣ : abstract_env_rel X Σ), ∥equality_terms Σ (Γ,,, stack_context π1) l1 l2∥) by struct l1 :=
  ...
  Equations(noeqns) _isconv_args (leq : conv_pb) (Γ : context)
    ...
    : ConversionResult (forall Σ (wfΣ : abstract_env_rel X Σ), ∥equality_terms Σ (Γ,,, stack_context π1)
  ...
  Next Obligation.
    ...
    eapply equality_terms_alt in X0 as (argsr&argsr'&[]).
  ...
  Next Obligation.
    ...
    2: now depelim s2.
    2: eapply whnf_conv_context; eauto.
    2: symmetry in X1; eapply context_equality_forget in X1; eauto.
    constructor.
    eapply equality_terms_red_conv; eauto.
    all:eapply into_red_terms; tea.
    1:fvs. 2:fvs.
  ...
  Next Obligation.
    ...
    2: eapply whnf_conv_context; eauto.
    2: { symmetry in X1. eapply context_equality_forget in X1; tea. }
    ...
    all: eapply whnf_conv_context in wh2; [|symmetry in X1; eapply context_equality_forget in X1; exact X1].
