...
Section Erase.
  ...
  Equations? erase (Γ : context) (t : term) (Ht : welltyped Σ Γ t) : E.term
      by struct t :=
  { erase Γ t Ht with (is_erasableb Σ Γ t Ht) :=
    {
      erase Γ _ Ht true := E.tBox;
      erase Γ (tRel i) Ht _ := E.tRel i ;
      erase Γ (tVar n) Ht _ := E.tVar n ;
      erase Γ (tEvar m l) Ht _ := E.tEvar m (erase_terms Γ l _) ;
...
Ltac iserasableb_irrel :=
  match goal with
  [ H : is_erasableb ?Σ ?Γ ?t ?wt = _ |- context [ is_erasableb _ _ _ ?wt'] ] => 
  rewrite -(is_erasableb_irrel wt wt') H
  end.
...
Proof.
  ...
  - cbn. rewrite is_box_tApp.
    ...
    * assumption.
  - cbn.
    destruct (is_erasableb_reflect Σ Γ (tInd kn u) Ht) => //.
    destruct Ht.
    sq. 
    eapply inversion_Ind in X as (? & ? & ? & ? & ? & ?) ; auto.
...
Proof.
  simp erase.
  unfold erase_clause_1.
  destruct is_erasableb => //.
  destruct t => //.
Qed.
...
Proof.
  ...
  all: try (destruct is_erasableb; [ now eauto using expanded | eauto using expanded]).
  ...
  - simp erase. destruct is_erasableb eqn:ise.
    ...
    + rewrite -ise -erase_equation_1.
      ...
      * eapply welltyped_mkApps_inv; sq; eauto. eexists; eauto.
  - destruct (is_erasableb Σ Γ (mkApps (tConstruct ind c u) args) wt) eqn:ise.
    + simp erase. rewrite ise /=. constructor.
    + destruct (is_erasableb_reflect Σ Γ (mkApps (tConstruct ind c u) args) wt) => //.
      destruct wt as (T & wt). rewrite erase_mkApps //.
      * eapply welltyped_mkApps_inv; sq; eauto. eexists; eauto.
      * intros ? ?. simp erase. unfold erase_clause_1.
        unfold is_erasableb. destruct is_erasable.
        ...
        -- sq. eapply erases_declared_constructor in H as H'; eauto. destruct H' as (? & ? & ? & ?). 
           ...
           { rewrite map_erase_length. destruct H5 as ((? & ? & ? & ? & ?) & ? & ?).
